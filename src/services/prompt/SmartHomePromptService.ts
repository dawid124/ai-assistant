...
[ChangeScene]
scene_name = (noc:night | tiwi:tv | iwi:tv | manual:manual | czil:warm | chill:warm | warm:warm | łarm:warm chil:war | auto:auto | ałto:auto) {name}
(scenę | scene | scena | szena) <scene_name>
(ustaw | zmień ) (scenę | scene | scena | szena) [na] <scene_name>

[ChangeAllLight]
percent = (0..100) {percent}
[ustaw] [zmień] (oświetlenie | światło) [na] <percent> [procent]
...
[IncreaseLight]
percent = (0..100) {percent}
więcej światła (){percent:10}
(rozjaśnij | zwiększ) (oświetlenie | światło | scenę) (){percent:20}

[DecreaseLight]
percent = (0..100) {percent}
(mniej | miej) (światła | szatła) (){percent:10}
(przyciemnij | zmniejsz) (oświetlenie | światło | scenę) (){percent:20}

[MacroRun]


[MoveBlind]
rolety [w] (dół:DIM_DOWN| górę:DIM_UP){mode} (){percent:33}
...
(włącz | odpal) [na tv] (kodi:tv-kodi | netflix:tv-netflix){id}            
(przycisz:tv-volume-down | podgłoś:tv-volume-up | wycisz:tv-volume-mute){id} (tv | telewizor)
</JSGF>

<prompt_examples>
USER: przycisz tv
AI:  { "intentName": "MacroRun",  "slots": [ {"entity": "id", "value": { "value": "tv-volume-down" } } ]}

USER: wycisz tv
AI:  { "intentName": "MacroRun",  "slots": [ {"entity": "id", "value": { "value": "tv-volume-mute" } } ]}
...
USER: wyłącz światło w jadalni
AI:  { "intentName": "ChangeIdLightOnOff",  "slots": [ {"entity": "ioId", "value": { "value": "group-dinner" } } ]}
